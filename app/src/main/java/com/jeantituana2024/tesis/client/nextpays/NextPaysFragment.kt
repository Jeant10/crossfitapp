...
class NextPaysFragment : Fragment() {

    private lateinit var binding: FragmentNextPaysBinding
    private lateinit var userPreferences: UserPreferences
    private lateinit var tokenPreferences: TokenPreferences
    private lateinit var adapterPayNext: AdapterNextPay
    private lateinit var payArrayList: ArrayList<PayInfo>

    // Mantén una referencia a la llamada Retrofit
    private var retrofitCall: Call<PaysInfoResponse>? = null

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding = FragmentNextPaysBinding.inflate(inflater,container, false)
        return binding.root
    }
    ...
    private fun setupUI() {
        ...
        binding.searchEt.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                try {
                    adapterPayNext.filter.filter(s)
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            }
    ...
    private fun loadPays() {
        ...
        if(token!=null){
            ...
            retrofitCall = RetrofitClient.instance.getPaysInfo("Bearer $token")

            retrofitCall?.enqueue(object: Callback<PaysInfoResponse> {
                override fun onResponse(
                    p0: Call<PaysInfoResponse>,
                    response: Response<PaysInfoResponse>
                ) {
                    ...
                    if(response.isSuccessful){
                        ...
                        adapterPayNext = AdapterNextPay(requireContext(), payArrayList)
                        binding.paysRv.adapter = adapterPayNext
                ...
                override fun onFailure(p0: Call<PaysInfoResponse>, t: Throwable) {
                    if (!isAdded) {
                        return // Salir si el Fragment no está adjunto
                    }
